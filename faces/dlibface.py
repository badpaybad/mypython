...
class CameraCapturer:
    ...
    def BuildDataKo1504(self):
        self.__FileFolder = os.path.dirname(os.path.realpath(__file__))
        fileRawVectorLbl = self.__FileFolder +"/vectorlbl.raw.txt"
        fileDlibModel= self.__FileFolder +"/svc.dlib.model"
        fileFacenetModel= self.__FileFolder +"/svc.facenet.model"
        
        folderRoot="D:/robot/robot-cloud/RobotCloud.KoDatasetDownloaderConsole/bin/Debug/netcoreapp3.1/faces"
        folderFound=self.__FileFolder+"/foundfaces"
            ...
            if(os.path.isdir(fd)):
                ...
                for f in files:
                    ...
                    if(os.path.isfile(f)):    
                        #print(f)                    
                        bmp = cv2.imread(f)                        
                        ffound=self.detector.detect_face(bmp)

                        if(len(ffound)<=0):
                            temp = cv2.rotate(bmp, cv2.ROTATE_90_CLOCKWISE)
                            ffound=self.detector.detect_face(temp)
                        if(len(ffound)<=0):
                        ...
                        if(len(ffound)>0):
                            ...
                            fileToWrite=folderFound+"/"+lbl+"_"+Path(f).name
                            cv2.imwrite(fileToWrite ,fcrop)
                            print (fileToWrite)                            
